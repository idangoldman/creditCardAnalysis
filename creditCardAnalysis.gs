...
var transactionDetailsTemplate = new makeStruct(
  "inputDate, fid, fname, type, nCard, billingMonth, transactionDate, name, amount, currency"
);
...
function get_max_data(f_handler, fid, categoriesTable) {
  ...
  for (var s = 0; s < sheets.length; s++) {
    ...
    for (var r = 0; r < lastRow; r++) {
      ...
      if (row[0].split("-").length == 3) {
        ...
        formattedRow.billingMonth = new Date(
          row[9].split("-")[2],
          row[9].split("-")[1] - 1,
          row[9].split("-")[0]
        ); //month is zero-based

        formattedRow.transactionDate = new Date(
          row[0].split("-")[2],
          row[0].split("-")[1] - 1,
          row[0].split("-")[0]
        ); //month is zero-based
  ...
  return {
    data: out_data,
    nRow: out_data.length,
    nCol: Object.keys(out_data[0]).length,
  };
}
...
function get_isracard_data(f_handler, fid, categoriesTable) {
  ...
  for (var r = 0; r < lastRow; r++) {
    ...
    if (row[1] == "מועד חיוב") {
      ...
      if (formattedRow.name == "TOTAL FOR DATE") {
        continue;
      }
...
function get_new_files_by_list(folderID, statusSheet) {
  var FilesIdColumnLetter = get_column_letter_from_name(
    get_row_values(statusSheet, 1),
    "File ID"
  );
  var listOfProcessesFiles = statusSheet
    .getRange([FilesIdColumnLetter, FilesIdColumnLetter].join(":"))
    .getValues();
...
function onOpen() {
  ...
  var menuItems = [{ name: "Check For New Files", functionName: "main" }];
